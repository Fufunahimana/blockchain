...
class CertificateApp:
    ...
    def create_form(self):
        ...
        self.upload_btn.grid(row=8, column=0, columnspan=2)
        ...
        self.toggle_qr_btn = tk.Button(self.root, text="Show/Hide QR Code", fg="red", width=25, font=("Times New Roman", 15, "bold"), bg="green", command=self.toggle_qr)
        self.toggle_qr_btn.grid(row=8, column=3)

        # Exit Button
        self.exit_btn = tk.Button(self.root, text="Exit", fg ="black",width=10,font=("Times New Roman", 15,"bold"),bg="red", command=self.root.quit)
        self.exit_btn.grid(row=8, column=4)
        ...
        self.treeview.grid(row=9, column=0, columnspan=5, padx=10, pady=10)
    ...
    def display_certificate_details(self, event):
        ...
        if cert:
            ...
            img = Image.open(cert['file_path']).resize((250, 200))
            self.current_image = ImageTk.PhotoImage(img)
            self.image_label.config(image=self.current_image, text="")
            
            # Display QR code
            qr_img = Image.open(cert['qr_path']).resize((250, 200))
    ...
    def toggle_qr(self):
        ...
        if self.qr_visible:
            self.qr_label.config(image="", text="QR Code")
        else:
            selected_item = self.treeview.selection()
            if selected_item:
                self.display_certificate_details(None)
