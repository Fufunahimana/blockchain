...
class CertificateApp:
    ...
    def create_form(self):
        ...
        self.image_label = tk.Label(self.root, text="No Image", bg="grey", width=100, height=30)
        self.image_label.grid(row=0, column=3, rowspan=4, padx=10)

        # QR Code Display
        self.qr_label = tk.Label(self.root, text="QR Code", bg="white", width=100, height=30)
        ...
        self.toggle_qr_btn.grid(row=9, column=2)
        ...
        self.treeview.grid(row=10, column=0, columnspan=4, padx=10, pady=10)
    ...
    def display_certificate_details(self, event):
        ...
        if cert:
            ...
            img = Image.open(cert['file_path']).resize((450, 300))
            self.current_image = ImageTk.PhotoImage(img)
            self.image_label.config(image=self.current_image, text="")

            # Display QR code
            qr_img = Image.open(cert['qr_path']).resize((450, 300))
    ...
    def toggle_qr(self):
        ...
        if self.qr_visible:
            self.qr_label.config(image="", text="No QR Code")
        else:
            selected_item = self.treeview.selection()
            if selected_item:
                self.display_certificate_details(None)
